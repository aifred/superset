...
def _list_with_page_size(page_size: int, page: int = 0) -> Mock:
    ...
    the ``.offset()``/``.limit()`` calls (the effective page and page size) can
    be inspected.
    """
    mock_query = Mock()
...
def test_list_negative_page_is_clamped_to_zero():
    ...
    negative = _list_with_page_size(page_size=10, page=-1)
    zero = _list_with_page_size(page_size=10, page=0)

    negative.offset.assert_called_once_with(0)
    zero.offset.assert_called_once_with(0)
    assert negative.offset.call_args == zero.offset.call_args


def test_list_nonnegative_page_is_unchanged():
    """A non-negative page produces the expected offset (page * page_size)."""
    mock_query = _list_with_page_size(page_size=10, page=3)

    mock_query.offset.assert_called_once_with(30)
