...
def _list_with_page_size(page_size: int, page: int = 0) -> Mock:
    ...
    the ``.offset()`` and ``.limit()`` calls can be inspected.
    """
    mock_query = Mock()
...
def test_list_negative_page_uses_first_page_offset():
    """A negative page uses the same offset as page zero."""
    negative_page_query = _list_with_page_size(50, page=-1)
    first_page_query = _list_with_page_size(50, page=0)

    negative_page_query.offset.assert_called_once_with(0)
    first_page_query.offset.assert_called_once_with(0)


def test_list_nonnegative_page_offset_is_unchanged():
    """A positive page retains the expected page-size-based offset."""
